...
def test_my_file(dict_list_my):
    assert my_file(1) == []
    assert my_file("") == []
    assert my_file("../Data/test.operations.json") == dict_list_my


def test_my_file_csv(test_info_csv, test_info_xlsx):
    ...
    assert info_xlsx[0] == test_info_xlsx


@patch("tests.test_utils.pd.read_csv")
def test_my_file_csv(test_info_csv, test_info_xlsx):
    Mock.return_value = pd.DataFrame()
    assert test_my_file_csv('foo') == []
...
def test_my_file_xlsx(mock_file):
